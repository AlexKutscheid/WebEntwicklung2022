<nav class="navbar navbar-expand-lg bg-light rounded mb-2">
    <div class="container-fluid">
        <div class="navbar-brand">Aufgabenplaner</div>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link" href="Projekte">Projekte</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"> Aktuelles Projekt</a>
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="AktuellesProjekt">Aktuelles Projekt</a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="Reiter">Reiter</a></li>
                        <li><a class="dropdown-item" href="Aufgaben">Aufgaben</a></li>
                        <li><a class="dropdown-item" href="Mitglieder">Mitglieder</a></li>
                    </ul>
                </li>
            </ul>
            <a class="btn btn-danger" href="Login/">Logout</a>

        </div>
</nav>

